...
from django_lms.settings import MEDIA_ROOT, MEDIA_URL
from lmsApp.forms import  UpdateProfile, UpdatePasswords,SaveBook
...
def context_data(request):
    ...
    context = {
        ...
        'page_name' : '',
        'page_title' : '',
        'system_name' : 'Systeme de Gestion de Documents',
        'topbar' : True,
        'footer' : True,
    }
        ...
        if form.is_valid():
            ...
            for field in form:
                for error in field.errors:
                    ...
                    form = SaveBook(request.POST,request.FILES)
                    if not resp['msg'] == '':
                        resp['msg'] += str('<br/>')
                    resp['msg'] += str(f'[{field.name}] {error}')
    else:
