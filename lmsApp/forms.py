...
class SaveBook(forms.ModelForm):
    ...
    class Meta:
        model = models.Books
        fields = ('isbn', 'sub_category', 'title', 'description', 'author', 'publisher', 'date_published', 'status','file_path' )
    ...
    def clean_title(self):
        ...
        try:
            if id.isnumeric():
                 book = models.Books.objects.exclude(id = id).get(title = self.cleaned_data['title'])
            else:
                 book = models.Books.objects.get(title = self.cleaned_data['title'])
        except:
            return self.cleaned_data['title']
        raise forms.ValidationError(f'{book.title} book Already Exists.')
